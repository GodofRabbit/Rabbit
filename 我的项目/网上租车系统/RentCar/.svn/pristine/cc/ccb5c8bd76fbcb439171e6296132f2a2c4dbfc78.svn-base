 //点击让提示信息隐藏
     function conceal(){
      ...
      function reCheckcode() {  
        ...
        $("#kaptchaImage").attr("src","http://localhost:8080/RentCar/createYZM.do?"+Math.random());  
      }
      //手机号验证
       function checkPhone(){
			var str =$("#phone").val();
			if(!(/^[1][35678][0-9]{9}$/.test(str))){
			    $("#phone_err").text("手机号码格式错误");
			    ...
			    $.ajax({
			      type:"post",
                  resultType:"json",
                  url:"http://localhost:8080/RentCar/checkPhone.do",
                  data:{phone:str},
