<template name="graceDate">
	<view class="grace-date" :hidden="show">
		...
		<view class="grace-date-week">
			<view v-for="(item, index) in weeks">{{item}}</view>
		</view>
		<view class="grace-date-days">
			<view :class="['items', currentDay == (cYear+'-'+cMonth+'-'+item.date) ? 'current' : '']" @tap="chooseDate" :data-date="cYear+'-'+cMonth+'-'+item.date" v-for="(item, index) in days" :style="{'background' : item == '' ? 'none' : ''}">
...
<style>
.grace-date{position:fixed; z-index:9998; vertical-align:top; left:0; top:0; width:700upx; padding:25upx; height:100%; background:#FFF; align-content:flex-start;}
.grace-date-header{flex-wrap:nowrap;}
.grace-date-header-btn{width:100upx; flex-shrink:0; font-size:38upx; line-height:88upx; justify-content:center;}
.grace-date-header-date{line-height:88upx; font-size:32upx; justify-content:center; font-weight:700;}
.grace-date-week{height:auto;}
.grace-date-week view{width:100upx; height:80upx; font-size:32upx; line-height:80upx; justify-content:center;}
.grace-date-days{padding:10upx 0; height:auto;}
.grace-date-days .items{width:98upx; justify-content:center; height:98upx; line-height:98upx; margin:1upx; background:#F1F2F3; font-size:30upx;}
