			...
			<view class="mx-modal">
				...
				<view class="mx-modal-body">
					...
					<view v-if="type=='datetime' || type=='time'">
						<!-- <view v-if="type!='time'" class="line-title">选择时间</view> -->
				...
				<view style="color: #666;font-size: 14px;">
					已选择:
					<text v-if="type!='time'">
						{{date.year<10?('0'+date.year):date.year}}年{{date.month<10?('0'+date.month):date.month}}月{{date.date<10?('0'+date.date):date.date}}日
					</text>
					<text v-if="type=='datetime' || type=='time'">
						{{time[0]<10?('0'+time[0]):time[0]}}:{{time[1]<10?('0'+time[1]):time[1]}}:{{time[2]<10?('0'+time[2]):time[2]}}
					</text>
				</view>
				<view>
